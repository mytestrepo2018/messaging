...
func init() {
	if argMessagingConfig != "" {
		var err error
		rcvr, err = messaging.GetReceiver(argMessaging)
		if err != nil {
			log.Fatal().
				Err(err).
		...
		err = rcvr.Init(argMessagingTopic, messaging.NatsIOConfig(argMessagingConfig, "A test NATS receiver", 5, 10))
		if err != nil {
			log.Fatal().
				Err(err).
